...
def create_blog_card(blog: BlogPost, blog_content_area: ui.column):
    ...
    ).style("box-shadow: 0 1px 2px rgba(0, 0, 0, 0.1); text-decoration: none;").on(
        "click", lambda: load_blog_content(blog.title, blog_content_area)
    ):
...
async def load_blog_content(blog_id: str, blog_content_area):
    ...
    with blog_content_area:
        ...
        ui.label(f"Published on: {blog.date.strftime('%Y-%m-%d')}").classes(
...
def init(app: FastAPI) -> None:
    ...
    async def blog_page():
        ...
        with ui.left_drawer(top_corner=False, bottom_corner=True).style(
            "background-color: #d7e3f4"
        ):
            ui.link("Home", "/").classes("text-lg font-bold p-2")
            ui.label("Blog Content").classes("text-lg font-bold p-2")

        with ui.right_drawer(elevated=True, fixed=False).style(
